...
gapminder %>%
  filter(year == 2015 & country %in% c("Sri Lanka", "Turkey","Russia",)) %>%
...
library(tidyverse)
library(dslabs)
data# smooth density plots - variable counts on y-axis
p <- gapminder %>% 
  filter(year == past_year & country %in% country_list) %>%
  mutate(group = ifelse(region %in% west, "West", "Developing")) %>%
  ggplot(aes(dollars_per_day, y = ..count.., fill = group)) +
...
titanic %>% ggplot(aes(Age, ..count.., fill = Sex)) + geom_density(alpha=0.2, bw=1)

head(titanic)
titanic %>% filter(Sex == "female" & Age >= 18 & Age <= 35 ) %>% nrow() / titanic %>% filter(Sex == "female") %>% nrow()
titanic %>% filter(Sex == "male" & Age >= 18 & Age <= 35 ) %>% nrow() / titanic %>% filter(Sex == "male") %>% nrow()

titanic %>% filter(Sex == "female" & Age == 40 ) %>% nrow() 
titanic %>% filter(Sex == "male" & Age == 40 ) %>% nrow()

titanic %>% filter(Sex == "female" & Age <= 17 ) %>% nrow() / titanic %>% filter(Sex == "female") %>% nrow() 
titanic %>% filter(Sex == "male" & Age <= 17 ) %>% nrow() / titanic %>% filter(Sex == "male") %>% nrow()
...
titanic %>% group_by(Sex, Survived) %>% summarize(count = n()) %>% 
  mutate(percent = count/sum(count)) 


titanic %>% group_by(Survived) %>% summarize(count = n()) %>% 
  mutate(percent = count/sum(count)) 


#%>%  ggplot(aes(Sex, Survived)) +   geom_col() +
  geom_text(aes(label = paste0(round(100 * percent, 1), "%")), vjust = -0.25)

titanic %>% ggplot(aes(Age, ..count.., fill = Survived)) + geom_density(alpha=0.2)
...
titanic %>% filter(Survived == 1) %>% ggplot(aes(Pclass, ..count..), position = position_fill()) + geom_bar()

titanic %>% filter(Survived == 1) %>% ggplot(aes(Pclass, ..count..)) + geom_bar()


titanic %>% ggplot(aes(Pclass, ..count..), position = position_fill()) + geom_bar()

total_survived = titanic %>% filter(Survived == 1) %>% count()

titanic %>% filter(Survived == 1) %>% group_by(Pclass) %>% summarize(portion_survived = n()/total_survived, amount_survived= n())                                                                      
